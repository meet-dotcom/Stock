...
public class RoleServiceImpl implements RoleService {
    ...
    public R<String> addRoleWithPermissions(RoleAddVo vo) {
        ...
        if (!CollectionUtils.isEmpty(permissionsIds)) {
            ...
            //批量插入角色权限集合
            int counts=sysRolePermissionMapper.addRolePermissionBatch(rps);
            if (counts==0) {
                throw new BusinessException(ResponseCode.ERROR.getMessage());
            }
        }
    ...
    public R<String> updateRoleWithPermissions(RoleUpdateVo vo) {
        ...
        int updateCount = sysRoleMapper.updateByPrimaryKeySelective(role);
        if (updateCount!=1) {
            throw new BusinessException(ResponseCode.ERROR.getMessage());
        }
        //2.删除当前角色关联的权限信息
        ...
        if (!CollectionUtils.isEmpty(permissionsIds)) {
            ...
            sysRolePermissionMapper.addRolePermissionBatch(rps);
        }
        return R.ok(ResponseCode.SUCCESS.getMessage());
    }
